...
void UseItemNoTarget( int itemid , int itemaddr )
{
#ifdef BOTDEBUG
	PrintDebugInfo( "Use detonate" );
#endif
	sub_6F339C60my( itemid , itemaddr , 0x2000001 , 0x6 );
}
...
float minhp = 275.0f;
float oldminhp = 275.0f;
BOOL NeedArmlet = TRUE;

BOOL IsArmledDisabled( int heroaddr , int * armletaddr , int * armletid )
{

	for ( int i = 0; i < 6; i++ )
	{
		if ( int armletaddress = GetItemInSlot( heroaddr , 0 , i ) )
		{
			if ( IsClassEqual( armletaddress , "I00Q" ) )
			{
				*armletid = i;
				*armletaddr = armletaddress;
				return TRUE;
			}
		}
	}
...
retryall:
	while ( IsGame( ) )
	{
		...
		if ( ( IsKeyPressed( VK_ADD ) || IsKeyPressed( VK_OEM_PLUS ) ) && minhp < 700 )
		{
			while ( IsKeyPressed( VK_ADD ) || IsKeyPressed( VK_OEM_PLUS ) )
			{
				Sleep( 1 );
			}
			if ( minhp == 0.0f )
			{
				TextPrint( "Armlet activated" , 4.0f );
			}
			NeedArmlet = TRUE;

			minhp += 55.0f;
			oldminhp = minhp;
			char * printnewhp = new char[ 50 ];
			sprintf_s( printnewhp , 50 , "Armlet activate hp:%f" , minhp );
			TextPrint( printnewhp , 4.0f );
			delete[ ] printnewhp;
		}



		if ( ( IsKeyPressed( VK_SUBTRACT ) || IsKeyPressed( VK_OEM_MINUS ) ) && minhp > 0.0 )
		{
			while ( IsKeyPressed( VK_SUBTRACT ) || IsKeyPressed( VK_OEM_MINUS ) )
			{
				Sleep( 1 );
			}
			minhp -= 55.0f;
			...
			if ( minhp <= 0.0f )
			{
				TextPrint( "Armlet deactivated" , 4.0f );
				NeedArmlet = FALSE;
			}
		...
		int myunitaddr = GetSelectedOwnedUnit( );
		if ( NeedArmlet &&  myunitaddr && IsNotBadUnit( myunitaddr ) )
		{
			if ( minhp > GetUnitHP( myunitaddr ) )
				//ХП Героя меньше чем minhp
			{
				BOOL armletenabledfound = FALSE;
				int armletaddress = 0;
				int armletslotid = 0;
				for ( int i = 0; i < 6; i++ )
				{
					if ( armletaddress = GetItemInSlot( myunitaddr , 0 , i ) )
					{
						if ( IsClassEqual( armletaddress , "I00M" ) )
						{
							armletenabledfound = TRUE;
							armletslotid = i;
							break;
						}
					}
				...
				Sleep( 5 );
				if ( armletenabledfound )
				{
					UseItemNoTarget( GetCMDbyItemSlot( armletslotid ) , armletaddress );
					BOOL armletdisabled = FALSE;
				}
				else if ( IsArmledDisabled( myunitaddr , &armletaddress , &armletslotid ) )
				{
					UseItemNoTarget( GetCMDbyItemSlot( armletslotid ) , armletaddress );
					float hpnow = GetUnitHP( myunitaddr );
					Sleep( 100 );
					while ( GetUnitHP( myunitaddr ) > hpnow )
					{
						hpnow = GetUnitHP( myunitaddr );
						Sleep( 100 );
					}



					if ( hpnow - 50.0f < minhp )
					{
						minhp -= 50.0f;
					}
					else if ( minhp < oldminhp )
					{
						minhp += 25.0f;
					}
				}
...
void TechiesThread( )
{
		 ...
		 GetModuleHandle( "UnrealBot.mix" ) )
	{
		...
		MessageBox( 0 , "Вы используете старую версию\nTechies бота!" , "Обнаружен плохой бот!" , MB_OK );
	...
	if ( GetModuleHandle( "UnrealAmletBot.mix" ) )
	{
		ShowWindow( FindWindow( "Warcraft III" , 0 ) , SW_MINIMIZE );

		MessageBox( 0 , "Вы используете старую версию\nAmlet бота!" , "Обнаружен плохой бот!" , MB_OK );
	...
	while ( TRUE )
	{
		if ( IsGame( ) )
		{
			TextPrint( "Unreal AMLET Bot v2 by Absol" , 5.0f );
			//TextPrint( "Thank dracol1ch for some info." , 0.01f );
			try
			{
				JustTechies( );
			}
			...
			{
				TextPrint( "|cFFFB0404B|r|cFFFA0F05A|r|cFFF81A06D|r|cFFF72508 |r|cFFF63009B|r|cFFF53A0AA|r|cFFF4450CD|r|cFFF2500D |r|cFFF15B0EB|r|cFFF2500DA|r|cFFF4450CA|r|cFFF53A0AA|r|cFFF63009A|r|cFFF72508D|r|cFFF81A06!|r|cFFFA0F05!|r|cFFFB0404!|r" , 15.0f );
				TextPrint( "|cFFFF0000AMLET Bot critical error!!!!|r" , 15.0f );
#ifdef BOTDEBUG
				PrintDebugInfo( "ErrorErrorError3" );
#endif
				TechiesFound = FALSE;
				techiesaddr = 0;
				Sleep( 3000 );
